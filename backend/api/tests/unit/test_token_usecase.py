...
class TestUseCaseAuthentication(SimpleTestCase):
    ...
    def test_generate_access_token(self):
        # Arrange / Act 
        token = self.token_use_case.generate_token("user_id", "user@example.com", ["group1", "group2"], "access_token")
        # Assert
        self.assertIsInstance(token, str)

    def test_generate_refresh_token(self):
        # Arrange / Act
        token = self.token_use_case.generate_token("user_id", "user@example.com", ["group1", "group2"], "refresh_token")
        # Assert
        self.assertIsInstance(token, str)
